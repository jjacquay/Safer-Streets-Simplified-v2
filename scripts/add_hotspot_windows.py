...
Method:
  ...
  3. Bucket crashes into fixed WINDOW_MI-mile windows from the start of the
     line coordinates (order as drawn; not a true route-direction guarantee,
     but stable and reproducible corridor-to-corridor).
...
def along_distance_mi(pt, lines):
    """Nearest projection of pt onto any segment of `lines`; returns distance-along-line
    in miles, measured from the start of that particular line (0-indexed coordinate order).
    Picks whichever line/segment is closest overall (mirrors the corridor's own buffer join).
    """
    ...
    for coords in lines:
        ...
        for i in range(len(coords) - 1):
            ...
            if d < best["dist_m"]:
                best = {"dist_m": d, "along_m": cum_m + t * seg_len_m}
            cum_m += seg_len_m
...
def corridor_hotspot(lines, crash_pts):
    ...
    for c in matched:
        along_mi = along_distance_mi((c["lon"], c["lat"]), lines)
